...
Q4. 登録した内容を外部へ書き出せますか？   <a id="A4"></a>
　　システム障害等で俳句アプリのデータが消えたり、他のアプリで利用したいときのために。
  
A4. はい、テキストデータで書き出せます。
　　　俳句だけの保存なら、メイン画面の右上の共有ボタンで、他のアプリへ保存すれば安心です。
   　俳句以外の覚書等も保存したいときは、検索・並び替え・絞り込み　のページ（Q６参照）の右上に共有ボタンで、書き出せば安心です。
